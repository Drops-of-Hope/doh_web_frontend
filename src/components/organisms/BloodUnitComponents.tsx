...
export const TestBadge = ({ status }: { status: 'pending' | 'pass' | 'fail' }) => {
  const badgeConfig = {
    pending: {
      bg: 'bg-gray-100',
      text: 'text-gray-600',
...
export const BloodUnitInfo = ({ bloodUnit }: { bloodUnit: BloodUnit }) => {
  ...
  const StatusBadge = ({ status }: { status: 'pending' | 'pass' | 'fail' }) => {
    ...
    return (
      <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm ${config.bg} ${config.text} border-1 ${config.border}`}>
        {config.icon}
        {config.label}
      </span>
    );
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <label className="text-sm font-medium text-gray-500 mb-2 block">Blood Unit ID</label>
          <span className="text-lg font-bold text-gray-900 font-mono">{bloodUnit.id}</span>
        </div>
        
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <label className="text-sm font-medium text-gray-500 mb-2 block">Blood Group</label>
          <BloodTypeBadge bloodGroup={bloodUnit.bloodGroup} />
        </div>
        
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <label className="text-sm font-medium text-gray-500 mb-2 block">Component Type</label>
          <span className="text-base font-semibold text-blue-800">{bloodUnit.componentType}</span>
        </div>
        
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <label className="text-sm font-medium text-gray-500 mb-2 block">Donation Location</label>
          <span className="text-base font-medium text-gray-900">{bloodUnit.donationLocation}</span>
        </div>
        
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <label className="text-sm font-medium text-gray-500 mb-2 block">Donation Date</label>
          <span className="text-base font-medium text-gray-900">{bloodUnit.donationDate}</span>
        </div>
        
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <label className="text-sm font-medium text-gray-500 mb-2 block">Volume</label>
          <span className="text-base font-semibold text-purple-800">{bloodUnit.volume}</span>
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition-colors ${
              !allCompulsoryCompleted
                ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                : 'bg-red-600 text-white hover:red-700'
            }`}
          >
