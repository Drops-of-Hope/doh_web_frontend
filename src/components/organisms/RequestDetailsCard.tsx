...
interface Props {
  ...
  transitStatus?: "not_started" | "ongoing" | "completed";
  transitDetails?: TransitDetails;
}

export const RequestDetailsCard: React.FC<Props> = ({
  request,
  requestStatus,
  transitStatus,
  transitDetails,
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Blood request from {request.hospital}
            </p>
          </div>
      ...
      {transitStatus && transitStatus !== "not_started" && transitDetails && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
          <h3 className="text-lg font-semibold mb-2">Transit Information</h3>
          <div className="mb-2">
            <span className="font-medium">Status:</span>{" "}
            {transitStatus === "ongoing" ? "Ongoing" : "Completed"}
          </div>
          <div className="mb-2">
            <span className="font-medium">Transporter:</span>{" "}
            {transitDetails.transporter}
          </div>
          <div className="mb-2">
          ...
          <div className="mb-2">
            ...
            {transitDetails.driverContact}
          </div>
          <div className="mb-2">
            <span className="font-medium">Emergency Contact:</span>{" "}
            {transitDetails.emergencyContact}
          </div>
          <div className="mb-2">
            <span className="font-medium">Vehicle:</span>{" "}
            {transitDetails.vehicle}
          </div>
          <div className="mb-2">
            <span className="font-medium">Departure Time:</span>{" "}
            {transitDetails.departureTime}
          </div>
          {transitDetails.note && (
            <div className="mb-2">
              <span className="font-medium">Note:</span> {transitDetails.note}
            </div>
          )}
        </div>
      )}
